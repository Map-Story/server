...
public class PostController {
    ...
    @GetMapping("/")
    public ResponseEntity<List<GetsPostResponse>> getAllPosts(@CookieValue(value = "Authorization") String token) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<GetPostResponse> getPostById(@PathVariable Long id, @CookieValue(value = "Authorization") String token) {
    ...
    public ResponseEntity<PostResponse> uploadImage(@RequestParam("image") MultipartFile image) throws IOException {
    ...
    public ResponseEntity<List<String>> uploadImages(@RequestPart(value = "images", required = false) List<MultipartFile> images) throws IOException {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<String> updatePost(@PathVariable Long id, @RequestBody UpdatePostRequest updatePostRequest, @CookieValue(value = "Authorization") String token) throws IOException {
    ...
    // 게시물 삭제
    // TODO : 사용자가 동일한 지 확인하는 것 추가하기
    @Operation(summary = "게시물 삭제", description = "S3 등록된 사진 삭제 + 데이터베이스에서 항목 삭제")
    @ApiResponses(value = {
